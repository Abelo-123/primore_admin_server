...
export async function sendSmsEthiopia({ phone = '251993960702', text, apiKey }) {
    ...
    if (!key) {
        console.warn(`[SMS-Ethiopia] Skipped sending to ${msisdn}: SMS_ETHIOPIA_API_KEY is missing.`);
        return {
            success: false,
            missing_key: true,
            error: 'SMS_ETHIOPIA_API_KEY is missing. Please save your key in Admin Settings or Render Environment.'
        };
    }

    console.log(`[SMS-Ethiopia] Sending SMS to ${msisdn} with key [${key.slice(0, 4)}***]...`);
    ...
    try {
        ...
        const isSuccess = res.ok && (data.status === 'success' || data.success === true);
        return {
            success: isSuccess,
            status_code: res.status,
            data,
            error: isSuccess ? null : (data.error || data.message || `HTTP ${res.status} ${res.statusText}`)
        };
    } catch (err) {
