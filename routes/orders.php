...
if ($route === '/orders/status') {
    ...
    try {
        $stmt = $pdo->prepare("
            SELECT id, api_order_id, cost, charge, quantity, status, reseller_cost, bot_id 
            FROM orders 
        ...
        foreach ($activeOrders as $order) {
            ...
            if ($info && isset($info['status'])) {
                ...
                if ($order['status'] !== $newStatus) {
                    $orderCharge = isset($order['cost']) && $order['cost'] !== null ? (float)$order['cost'] : (float)($order['charge'] ?? 0);
                    $refundAmt = 0.0;
                    $resellerRefund = 0.0;

                    if (in_array($newStatus, $terminalStatuses)) {
                        $refundAmt = $orderCharge;
