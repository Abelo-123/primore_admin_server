...
/**
 * Main API Router Entry Point for Admin App
 */
...
if (!$route) {
    ...
    $scriptDir = rtrim(dirname($_SERVER['SCRIPT_NAME']), '/'); // e.g. /server or /api
...
// Strip /api prefix if present (Vite proxies requests with /api)
if (strpos($route, '/api/') === 0) {
    $route = substr($route, 4); // strip '/api'
} elseif ($route === '/api') {
    $route = '/';
}
...
// Merge query parameters, POST body, and JSON input for easier retrieval
